...
class AuthClient(Client):
    ...
    def patch_json(self, path, data=None):
        return self.patch(
            path,
            data=json.dumps(data or {}),
            content_type="application/json",
...
# ── 测试数据工厂 ──
# 数字学号夹具统一使用 2099 年前缀，表示不对应任何当前真实用户的合成身份。
...
class BnuTestCase(TestCase):
    ...
    def setUp(self):
        from ..views.utils import _clear_category_preload
        _clear_category_preload()  # 清理上次测试的线程局部 preload 缓存
