...
class PermissionMatrixTest(BnuTestCase):
    ...
    def test_auto_approve_toggle_permissions(self):
        """POST /api/admin/users/{uid}/auto-approve/（仅 super_admin，提权修复）"""
        # 非超管一律 403（修复前任何登录用户可开关他人自动托管）
        for u in [self.user, self.sub_mod, self.mod]:
            self._test_endpoint(u, "POST",
                f"/api/admin/users/{self.user.id}/auto-approve/",
                403)
        # 超管可操作：目标非版主/小版主 → 400；目标是版主 → 200
        resp = self._test_endpoint(self.admin, "POST",
            f"/api/admin/users/{self.user.id}/auto-approve/", 400)
        mod_target = create_user("mod_target", role="moderator")
        resp = self._test_endpoint(self.admin, "POST",
            f"/api/admin/users/{mod_target.id}/auto-approve/", 200)
        self.assertTrue(resp.json().get("ok"))
